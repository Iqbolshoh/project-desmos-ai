...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.questions.update', $question->id) }}" method="POST" class="card p-6 border-[var(--border-strong)] bg-[var(--bg-overlay)] space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="type" class="input w-full bg-black/50" required>
                    <option value="mcq" @selected($question->type === 'mcq')>Test (MCQ)</option>
                    <option value="free_response" @selected($question->type === 'free_response')>Ochiq savol (Free Response)</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="difficulty" class="input w-full bg-black/50" required>
                    <option value="easy" @selected($question->difficulty === 'easy')>Oson</option>
                    <option value="medium" @selected($question->difficulty === 'medium')>O'rtacha</option>
                    <option value="hard" @selected($question->difficulty === 'hard')>Qiyin</option>
                </select>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="options" value="{{ $question->options ? json_encode($question->options) : '' }}" class="input w-full bg-black/50 font-mono text-xs" placeholder='{"A":"1", "B":"2", "C":"3", "D":"4"}'>
            </div>
        </div>
