...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-[var(--text-secondary)] mt-1">Admin management panel for SAT Math question repository.</p>
        </div>
        <a href="{{ route('admin.questions.create') }}" class="btn-primary flex items-center gap-2">
            <x-lucide-plus class="w-4 h-4" /> New Question
        </a>
    </div>
        ...
        <table class="w-full text-left text-sm text-[var(--text-secondary)]">
            <thead class="bg-black/40 text-[var(--text-muted)] font-mono text-xs uppercase tracking-wider">
                <tr>
                    ...
                    <th class="px-6 py-4">Difficulty / Type</th>
                    <th class="px-6 py-4">Prompt Snippet</th>
            ...
            <tbody class="divide-y divide-[var(--border-subtle)]">
                ...
                <tr class="hover:bg-white/5 transition-colors">
                    ...
                    <td class="px-6 py-4 font-medium text-white flex items-center gap-2">
                        ...
                        {{ $q->topic->name ?? 'Unknown' }}
                    </td>
                    <td class="px-6 py-4">
                        <span class="badge badge-accent uppercase text-[0.65rem] font-mono">{{ $q->difficulty }}</span>
                        @if($q->is_diagnostic)
                            <span class="badge bg-yellow-500/10 text-yellow-500 uppercase text-[0.65rem] ml-1 font-mono">Diagnostic</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 flex gap-2">
                        <a href="{{ route('admin.questions.edit', $q->id) }}" class="text-[var(--accent-hover)] hover:text-[var(--accent-alt)] transition-colors p-2 rounded-lg hover:bg-[var(--accent-soft)]" title="Edit Question">
                            <x-lucide-pencil class="w-4 h-4" />
                        </a>
                        <form action="{{ route('admin.questions.destroy', $q->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this question?');">
                            @csrf
                            @method('DELETE')
                            <button class="text-red-400 hover:text-red-300 transition-colors p-2 rounded-lg hover:bg-red-500/10" title="Delete Question">
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-[var(--text-muted)]">
                        No questions have been added to the repository yet.
                    </td>
                </tr>
                @endforelse
            </tbody>
