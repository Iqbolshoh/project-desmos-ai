...
<div class="space-y-8 max-w-6xl mx-auto">
    ...
    {{-- Welcome Card --}}
    <div class="relative overflow-hidden card border-[var(--accent-border)] bg-gradient-to-br from-[var(--bg-raised)] to-black rounded-3xl p-8 sm:p-10 shadow-2xl">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="px-3 py-1 bg-[var(--accent-soft)] text-[var(--accent-hover)] text-xs font-bold rounded-lg border border-[var(--accent-border)] uppercase tracking-wider font-mono">
            ...
            <div class="text-right flex-shrink-0 bg-black/40 p-4 rounded-2xl border border-[var(--border-strong)] backdrop-blur-md">
                <div class="flex items-center gap-2 text-[var(--text-muted)] mb-1">
                    ...
                    <p class="text-sm font-bold font-mono">{{ now()->format('M d, Y') }}</p>
                </div>
    ...
    <div class="space-y-6">
        <div class="flex items-center gap-3">
            <h3 class="text-lg font-extrabold text-white">SAT Prep & Mastery</h3>
    ...
    {{-- Platform Stats (Admin / Staff) --}}
    @canany(['users.view', 'roles.view'])
    <div class="space-y-6 mt-12">
        <div class="flex items-center gap-3">
            <h3 class="text-lg font-extrabold text-white">Platform Administration</h3>
            <div class="h-px flex-1 bg-gradient-to-r from-[var(--border-strong)] to-transparent"></div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            @can('users.view')
            <div class="card p-6 border-[var(--border-strong)] rounded-2xl hover:border-[var(--accent-border)] transition-all duration-300 group hover:-translate-y-1 hover:shadow-xl shadow-lg relative overflow-hidden">
                ...
                <div class="flex items-start justify-between relative z-10">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-widest text-[var(--text-muted)] group-hover:text-[var(--accent-hover)] transition-colors font-mono">Total Users</p>
                        <p class="mt-2 text-4xl font-extrabold text-white font-mono">{{ User::count() }}</p>
                    </div>
                ...
                <a href="{{ route('users.index') }}" class="mt-5 inline-flex items-center gap-1.5 text-sm font-bold text-[var(--accent-hover)] hover:text-white transition-colors relative z-10">
                    Manage Users <x-lucide-arrow-right class="w-4 h-4" />
                </a>
            </div>
            @endcan

            @can('roles.view')
            <div class="card p-6 border-[var(--border-strong)] rounded-2xl hover:border-indigo-500/30 transition-all duration-300 group hover:-translate-y-1 hover:shadow-xl shadow-lg relative overflow-hidden">
                ...
                <div class="flex items-start justify-between relative z-10">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-widest text-[var(--text-muted)] group-hover:text-indigo-400 transition-colors font-mono">Active Roles</p>
                        <p class="mt-2 text-4xl font-extrabold text-white font-mono">{{ Role::count() }}</p>
                    </div>
                ...
                <a href="{{ route('roles.index') }}" class="mt-5 inline-flex items-center gap-1.5 text-sm font-bold text-indigo-400 hover:text-white transition-colors relative z-10">
                    Manage Roles <x-lucide-arrow-right class="w-4 h-4" />
                </a>
            </div>
            @endcan

            <div class="card p-6 border-[var(--border-strong)] rounded-2xl relative overflow-hidden bg-gradient-to-br from-[var(--bg-raised)] to-black group shadow-lg">
                <div class="absolute top-0 right-0 w-32 h-32 bg-[var(--gold-soft)] rounded-bl-full opacity-10 -mr-10 -mt-10"></div>
                <div class="flex items-start justify-between relative z-10">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-widest text-[var(--text-muted)] font-mono">System Status</p>
                        <p class="mt-2 text-xl font-extrabold text-white">Desmos AI v2.1</p>
                    </div>
                ...
                <div class="mt-5 flex items-center gap-2 text-sm text-[var(--text-secondary)] relative z-10">
                    ...
                    Laravel {{ app()->version() }} • All Systems Operational
                </div>
            </div>
        </div>

        {{-- Quick actions --}}
        <div class="card p-6 border-[var(--border-strong)] rounded-2xl mt-6 bg-[var(--bg-overlay)] shadow-xl">
            <h3 class="text-sm font-bold text-white mb-5 uppercase tracking-wider flex items-center gap-2 font-mono">
                <x-lucide-zap class="w-4 h-4 text-[var(--accent)]" /> Quick Management Actions
            </h3>
            <div class="flex flex-wrap gap-3">
                @can('users.create')
                <a href="{{ route('users.create') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-[var(--accent)] to-[var(--accent-alt)] text-white font-bold rounded-xl shadow-lg hover:brightness-110 transition-all">
                    <x-lucide-user-plus class="w-4 h-4" /> Add User
                </a>
                @endcan
                @can('roles.create')
                <a href="{{ route('roles.create') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 font-bold rounded-xl hover:bg-indigo-500 hover:text-white transition-all">
                    <x-lucide-shield-plus class="w-4 h-4" /> Add Role
                </a>
                @endcan
                @can('roles.view')
                <a href="{{ route('roles.index') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-black/20 border border-[var(--border-subtle)] text-[var(--text-secondary)] font-bold rounded-xl hover:text-white hover:border-[var(--border-strong)] transition-all">
                    <x-lucide-shield-check class="w-4 h-4" /> Role Permissions
                </a>
                @endcan
            </div>
        </div>
